...
class TestAlreadyClean:
    def test_clean_field_unchanged_2d(self):
        ...
        phi = _run_serial(dvf)
        np.testing.assert_allclose(phi, phi0, atol=1e-12, err_msg="Clean field was modified")
...
class TestSingleFold:
    def test_single_interior_fold_resolved(self):
        ...
        phi = _run_serial(dvf)
        assert _converged(phi), f"Single interior fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_single_fold_small_grid(self):
        ...
        assert _converged(phi), f"5×5 single fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_single_fold_3x3_grid(self):
        ...
        assert _converged(phi), f"3×3 single fold not resolved; min_jdet={_min_jdet(phi):.4f}"
...
class TestBorderFolds:
    def test_fold_on_top_row(self):
        ...
        assert _converged(phi), f"Top-row fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_fold_on_bottom_row(self):
        ...
        dvf[2, 0, -2, :] = -3.0
        phi = _run_serial(dvf)
        assert _converged(phi), f"Bottom-row fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_fold_on_left_column(self):
        ...
        dvf[1, 0, :, 1] = -3.0
        phi = _run_serial(dvf)
        assert _converged(phi), f"Left-column fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_fold_at_corner(self):
        ...
        dvf[1, 0, 1, 0] = -3.0
        phi = _run_serial(dvf, max_iterations=2000)
        assert _converged(phi), f"Corner fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_fold_entire_border(self):
        ...
        dvf[2, 0, -2, :] = -3.0
        phi = _run_serial(dvf, max_iterations=2000)
        assert _converged(phi), f"Full-border fold not resolved; min_jdet={_min_jdet(phi):.4f}"
...
class TestLargeRegions:
    def test_large_contiguous_region(self):
        ...
        assert _converged(phi), f"Large region not resolved; min_jdet={_min_jdet(phi):.4f}"
...
class TestNonSquareGrids:
    ...
    def test_non_square_fold(self, H, W):
        ...
        dvf[2, 0, cy, cx + 1] = -2.5
        phi = _run_serial(dvf, max_iterations=2000)
        assert _converged(phi), f"{H}×{W} grid: fold not resolved; min_jdet={_min_jdet(phi):.4f}"

    def test_tall_border_fold(self):
        """Tall grid (30×5) with a border fold — window must grow to 30×5."""
        dvf = _border_fold_dvf_2d(30, 5)
        phi = _run_serial(dvf, max_iterations=2000)
        assert _converged(phi), f"30×5 border fold not resolved; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_wide_border_fold(self):
        ...
        dvf[1, 0, :, 1] = -3.0
        phi = _run_serial(dvf, max_iterations=2000)
        assert _converged(phi), f"5×30 border fold not resolved; min_jdet={_min_jdet(phi):.4f}"
...
class TestStallEscalation:
    ...
    def test_oscillation_does_not_loop_forever(self):
        ...
        dvf[2, 0, 6, 7] = -3.0
        phi = _run_serial(dvf, max_iterations=3000)
        assert _converged(phi), f"Oscillating folds not resolved; min_jdet={_min_jdet(phi):.4f}"
...
class TestReproducibility:
    def test_deterministic_output(self):
        ...
        np.testing.assert_array_equal(phi1, phi2, err_msg="Non-deterministic output")
    ...
    def test_idempotent_on_already_corrected(self):
        ...
        np.testing.assert_allclose(
            phi1, phi2, atol=1e-8, err_msg="Second pass modified already-corrected phi"
        )
...
class TestConstraintModes:
    def test_shoelace_mode_converges(self):
        ...
        assert _converged(phi), f"Shoelace mode not converged; min_jdet={_min_jdet(phi):.4f}"

    def test_injectivity_mode_converges(self):
        dvf = np.zeros((3, 1, 12, 12), dtype=np.float64)
        dvf[2, 0, 5, 5] = 2.5
        dvf[2, 0, 5, 6] = -2.5
        phi = _run_serial(dvf, enforce_injectivity=True, injectivity_threshold=0.05)
        assert _converged(phi), f"Injectivity mode not converged; min_jdet={_min_jdet(phi):.4f}"
    ...
    def test_both_constraints_converge(self):
        ...
        dvf[2, 0, 5, 6] = -2.5
        phi = _run_serial(
            dvf, enforce_shoelace=True, enforce_injectivity=True, injectivity_threshold=0.05
        )
        assert _converged(phi), f"Dual constraint mode not converged; min_jdet={_min_jdet(phi):.4f}"
...
class TestSpatialHelpers:
    ...
    def test_neg_jdet_bounding_window_single_pixel(self):
        ...
        size, _center = neg_jdet_bounding_window(jm, (5, 5), THRESHOLD, ERR_TOL)
    ...
    def test_neg_jdet_bounding_window_precomputed_labels(self):
        ...
        size2, center2 = neg_jdet_bounding_window(jm, (5, 5), THRESHOLD, ERR_TOL, labeled=labeled)
        assert size1 == size2 and center1 == center2, "Precomputed labels give different result"
