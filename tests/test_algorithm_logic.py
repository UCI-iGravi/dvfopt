...
class TestConnectedComponentIsolation:
    ...
    def test_bounding_window_does_not_cross_to_distant_cluster(self):
        ...
        from dvfopt.core.slsqp_windowed.spatial import get_nearest_center, neg_jdet_bounding_window
        ...
        _, _cy1, cx1 = get_nearest_center(center1, (1, H, W), size1)
        _hy1, hx1 = size1[0] // 2, size1[1] // 2
        hx1_hi = size1[1] - hx1
    ...
    def test_bounding_window_center_is_bbox_center_not_worst_pixel(self):
        ...
        assert cx >= 5, f"Window center x ({cx}) should be near bbox center, not worst pixel col 3"
...
class TestWriteBackLogic:
    ...
    def test_unpadded_writeback_modifies_full_window_including_boundary(self):
        ...
        np.testing.assert_allclose(
            window, 7.0, atol=1e-12, err_msg="Unpadded write-back must cover full window"
        )
...
class TestEdgeFlagsLogic:
    ...
    def _flags(self, cy, cx, size, H=10, W=10, max_size=None):
        from dvfopt.core.slsqp_windowed.spatial import _edge_flags
...
class TestQualityMapExact:
    ...
    def test_quality_map_for_identity_is_exactly_one(self):
        ...
        np.testing.assert_allclose(
            qm, 1.0, atol=1e-10, err_msg="Quality map for identity field should be exactly 1"
        )
    ...
    def test_quality_map_minimum_equals_minimum_of_contributing_metrics(self):
        ...
        assert qm.min() <= jdet.min() + 1e-12, "quality_map.min() cannot exceed jdet.min()"
...
class TestThresholdBoundaryCondition:
    ...
    def test_pixel_at_exactly_boundary_counts_as_negative(self):
        ...
        assert neg_count == 1, f"Expected 1 pixel at boundary to count as negative, got {neg_count}"
    ...
    def test_algorithm_stops_when_all_pixels_above_threshold(self):
        ...
        np.testing.assert_array_equal(
            phi, np.zeros((2, 8, 8)), err_msg="Clean field should not be modified at all"
        )
    ...
    def test_algorithm_runs_if_exactly_one_pixel_at_boundary(self, tmp_path):
        ...
        from dvfopt.core.slsqp_windowed.coordinator import _save_results
...
class TestWindowGrowthMechanism:
    ...
    def test_window_sized_to_component_for_hard_fold(self, tmp_path):
        ...
        assert len(data_lines) >= 1, "Expected at least one window size in window_counts.csv"
    ...
    def test_larger_windows_appear_in_counts_for_hard_fold(self, tmp_path):
        ...
        assert has_large, f"No window larger than 3x3 used; sizes: {window_sizes}"
    ...
    def test_simple_fold_uses_at_least_one_window(self, tmp_path):
        ...
        assert total_calls >= 1, f"Expected at least 1 optimizer call, got {total_calls}"
