...
class TestPhiSubFlatRoundtrip:
    ...
    def test_extract_apply_roundtrip_unpadded(self):
        ...
        np.testing.assert_allclose(
            flat2, modified, atol=1e-12, err_msg="Roundtrip failed for unpadded sub-window"
        )
...
class TestConstraintJacobians:
    ...
    def test_jdet_constraint_jacobian_2d_interior(self):
        ...
        def fn(p):
            return jacobian_constraint(p, sub_size, exclude_boundaries=True)

        n_out = len(fn(phi))
        J_analytical = jdet_constraint_jacobian_2d(phi, sub_size, exclude_boundaries=True).toarray()
        J_fd = self._fd_jacobian(fn, phi, n_out)
        np.testing.assert_allclose(
            J_analytical, J_fd, atol=self.ATOL, err_msg="Jdet Jacobian 2D (interior) mismatch"
        )
    ...
    def test_jdet_constraint_jacobian_2d_full(self):
        ...
        def fn(p):
            return jacobian_constraint(p, sub_size, exclude_boundaries=False)

        n_out = len(fn(phi))
        J_analytical = jdet_constraint_jacobian_2d(
            phi, sub_size, exclude_boundaries=False
        ).toarray()
        J_fd = self._fd_jacobian(fn, phi, n_out)
        np.testing.assert_allclose(
            J_analytical, J_fd, atol=self.ATOL, err_msg="Jdet Jacobian 2D (full) mismatch"
        )
    ...
    def test_shoelace_constraint_jacobian_2d(self):
        ...
        def fn(p):
            return shoelace_constraint(p, sub_size, exclude_boundaries=True)

        n_out = len(fn(phi))
        J_analytical = shoelace_constraint_jacobian_2d(
            phi, sub_size, exclude_boundaries=True
        ).toarray()
        J_fd = self._fd_jacobian(fn, phi, n_out)
        np.testing.assert_allclose(
            J_analytical, J_fd, atol=self.ATOL, err_msg="Shoelace Jacobian mismatch"
        )
    ...
    def test_injectivity_constraint_jacobian_2d(self):
        ...
        def fn(p):
            return injectivity_constraint(p, sub_size, exclude_boundaries=True)

        n_out = len(fn(phi))
        J_analytical = injectivity_constraint_jacobian_2d(
            phi, sub_size, exclude_boundaries=True
        ).toarray()
        J_fd = self._fd_jacobian(fn, phi, n_out)
        np.testing.assert_allclose(
            J_analytical, J_fd, atol=self.ATOL, err_msg="Injectivity Jacobian mismatch"
        )
    ...
    def test_jdet_constraint_jacobian_3d(self):
        ...
        def fn(p):
            return jacobian_constraint_3d(p, sub_size)

        n_out = len(fn(phi))
        J_analytical = jdet_constraint_jacobian_3d(phi, sub_size).toarray()
        J_fd = self._fd_jacobian(fn, phi, n_out)
        np.testing.assert_allclose(
            J_analytical, J_fd, atol=self.ATOL, err_msg="Jdet Jacobian 3D mismatch"
        )
...
class TestJacobianIdentity:
    def test_zero_displacement_gives_jdet_one(self):
        ...
        np.testing.assert_allclose(jdet, 1.0, atol=1e-12, err_msg="Zero phi should give Jdet=1")
    ...
    def test_uniform_displacement_gives_jdet_one(self):
        ...
        jdet = jacobian_det2D(phi)
        np.testing.assert_allclose(
            jdet, 1.0, atol=1e-12, err_msg="Constant displacement should give Jdet=1"
        )
    ...
    def test_linear_compression_gives_correct_jdet(self):
        ...
        np.testing.assert_allclose(
            interior_jdet, s * s, atol=1e-10, err_msg="Linear compression Jdet wrong"
        )
    ...
    def test_selected_windows_non_overlapping(self):
        ...
        for i, (_, (_, cy1, cx1), sz1) in enumerate(batch):
            for j, (_, (_, cy2, cx2), sz2) in enumerate(batch):
                if i >= j:
                    continue
                assert not self._windows_overlap_including_gap(cy1, cx1, sz1, cy2, cx2, sz2), (
...
class TestBoundingWindowEdgeCases:
    ...
    def test_negative_pixel_at_last_column(self):
        ...
        assert 0 <= cy < H and 0 <= cx < W, f"Raw center ({cy},{cx}) outside grid {H}×{W}"
    ...
    def test_whole_grid_negative_returns_grid_size(self):
        ...
        size, _center = neg_jdet_bounding_window(jm, (4, 4), THRESHOLD, ERR_TOL)
...
class TestStallCountPurge:
    ...
    def test_stale_stall_key_purged(self):
        ...
        stall_counts = {
            k: v for k, v in stall_counts.items() if quality[0, k[0], k[1]] <= THRESHOLD - ERR_TOL
        }
...
class TestQualityMap:
    ...
    def test_quality_le_jdet_with_shoelace(self):
        ...
        jdet = jacobian_det2D(phi)
        qm = _quality_map(
            phi, enforce_shoelace=True, enforce_injectivity=False, jacobian_matrix=jdet
        )
    ...
    def test_quality_le_jdet_with_injectivity(self):
        ...
        jdet = jacobian_det2D(phi)
        qm = _quality_map(
            phi, enforce_shoelace=False, enforce_injectivity=True, jacobian_matrix=jdet
        )
    ...
    def test_quality_equals_jdet_when_both_false(self):
        ...
        jdet = jacobian_det2D(phi)
        qm = _quality_map(
            phi, enforce_shoelace=False, enforce_injectivity=False, jacobian_matrix=jdet
        )
