...
component order. Parity is checked to 1e-10 against the numpy kernel by
``runners/_verify_gpu_tet_parity.py`` before any benchmarking.
    ...
    *,
    ...
    for outer in range(n_outer):
        ...
        for _ in range(n_inner):
            ...
            g = V - tgt
            p = torch.where(
                g <= lam / mu, -lam * g + 0.5 * mu * g * g, -0.5 * lam * lam / mu
            )
            data = (
