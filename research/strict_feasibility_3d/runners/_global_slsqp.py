        ...
        for dz in range(-k_ring, k_ring + 1):
            for dy in range(-k_ring, k_ring + 1):
                for dx in range(-k_ring, k_ring + 1):
                    ...
                    if 0 <= nz < cells_max[0] and 0 <= ny < cells_max[1] and 0 <= nx < cells_max[2]:
...
def main():
    ...
    if plateau_path.exists():
        print(f'\n=== Loading saved M10Tet plateau from {plateau_path} ===', flush=True)
    ...
    for k_ring in [2, 3]:
        ...
        cubes, free_corners, x0 = build_global_slsqp_problem(phi_after_m10, k_ring=k_ring)
        print(
            f'  Problem size: cubes={len(cubes)}, '
            f'free_corners={len(free_corners)}, DOF={3 * len(free_corners)}, '
            f'constraints={6 * len(cubes)}',
            flush=True,
        )
        ...
        print(f'  SLSQP success={res.success}, fun={res.fun:.4f}, iter={res.nit}', flush=True)
        print(f'  message: {res.message}', flush=True)
        if res is not None and res.success:
            phi_after_slsqp = apply_x(res.x)
            n, b, _ = report(phi_after_slsqp, f'  after global SLSQP k={k_ring}', phi_input)
            ...
            if n <= 2:
                ...
                print(f'\n  M10Tet @ 0.012 recovery on k={k_ring} result...', flush=True)
                t0 = time.time()
                final = m10tet(phi_after_slsqp, 0.012)
                wall_rec = time.time() - t0
                print(f'  recovery wall={wall_rec:.1f}s ({wall_rec / 60:.1f} min)', flush=True)
                n, b, mn = report(final, f'  FINAL after k={k_ring}+recovery', phi_input)
                if n == 0 and b == 0:
                    np.save(OUTPUT / 'b0039_z0_15_strict_via_global.npy', final)
                    print(f'  *** STRICT FEASIBLE via global k={k_ring}+recovery ***', flush=True)
                    return
        else:
