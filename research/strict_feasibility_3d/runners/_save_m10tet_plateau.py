...
def main():
    ...
    if plateau_path.exists():
        ...
        print(
            f'  n_neg={int((V <= 0).sum())} n<0.01={int((V < 0.01 - 1e-5).sum())} '
            f'min_T={V.min():+.6f}',
            flush=True,
        )
        return
    ...
    print(f'Input: n_neg={int((V0 <= 0).sum())} n<0.01={int((V0 < 0.01 - 1e-5).sum())}', flush=True)
    ...
    print(
        f'Result: n_neg={int((V <= 0).sum())} n<0.01={int((V < 0.01 - 1e-5).sum())} '
        f'min_T={V.min():+.6f}',
        flush=True,
    )
