...
def plot_cube_3d(ax, pos, title, highlight_edges=None, color_edges_by_length=True):
    ...
    coll = Poly3DCollection(polys, alpha=0.06, facecolor='steelblue', edgecolor='none')
    ...
    # Corner scatter.
    ax.scatter(
        pos[:, 0], pos[:, 1], pos[:, 2], c='red', s=80, edgecolors='black', linewidth=1, zorder=20
    )
    for i in range(8):
        ax.text(pos[i, 0], pos[i, 1], pos[i, 2], f' {i}', fontsize=10, fontweight='bold')
    ax.set_title(title)
    ax.set_xlabel('Z')
    ax.set_ylabel('Y')
    ax.set_zlabel('X')
...
def fig_geometry_comparison():
    ...
    plot_cube_3d(
        ax1, pos_ideal, 'IDEAL unit cube\n(undeformed reference)', color_edges_by_length=False
    )
    ...
    plt.suptitle('Same cube at lattice (z=1, y=215, x=220) — three states', fontsize=13)
    plt.tight_layout()
...
def fig_zoom_crushed_edge():
    ...
    plot_cube_3d(
        ax1, pos, 'BEST_1fold cube\n(2,6) edge collapsed to 0.03 voxels', highlight_edges=[(2, 6)]
    )
    ...
    edges_to_show = [
        ...
        (2, 0),
        (2, 3),
        (2, 7),  # other connections from 2 (diagonals included)
        (6, 4),
        (6, 7),
        (6, 0),  # other connections from 6
        (2, 1),
        (2, 5),
        (6, 1),
        (6, 3),
    ...
    plt.suptitle('The crushed edge — geometric source of the topological deadlock', fontsize=13)
    plt.tight_layout()
...
def fig_rank_deficiency_analysis():
    ...
    titles = ['sigma_3 at u=0 (x-min)', 'sigma_3 at u=0.5 (mid)', 'sigma_3 at u=1 (x-max)']
    slices = [sigma3[:, :, 0], sigma3[:, :, N // 2], sigma3[:, :, -1]]
    for ax, t, s in zip(axes[0], titles, slices):
        im = ax.imshow(
            s, cmap='magma', vmin=0, vmax=sigma3.max(), origin='lower', extent=[0, 1, 0, 1]
        )
        ax.set_title(t + f'\nmin={s.min():.3f} (full cube: {sigma3.min():.3f})')
        ax.set_xlabel('v (y-param)')
        ax.set_ylabel('w (z-param)')
    ...
    for ax, t, s in zip(axes[1], titles, slices):
        im = ax.imshow(s, cmap='RdBu_r', vmin=-vmax, vmax=vmax, origin='lower', extent=[0, 1, 0, 1])
        ax.set_title(
            t + f'\nmin={s.min():+.3f} max={s.max():+.3f}  #neg={int((s < 0).sum())}/{s.size}'
        )
        ax.set_xlabel('v (y-param)')
        ax.set_ylabel('w (z-param)')
    ...
    for ax in axes.flat:
        ...
        ax.plot(
            1.0, 0.0, 'o', color='cyan', mew=2, mec='black', mfc='none', markersize=14, zorder=10
        )
        # corner 6: (u, v, w) = (0, 1, 1) so v=1, w=1; appears at u=0 slice.
        ax.plot(
            1.0, 1.0, 'o', color='cyan', mew=2, mec='black', mfc='none', markersize=14, zorder=10
        )

    plt.suptitle(
        'Rank-deficiency analysis: sigma_3 (top) and det(J) (bottom)\n'
...
def fig_feasibility_landscape():
    ...
    axes[0].contour(
        shifts, shifts, min_v, levels=[0.01], colors='gray', linewidths=1, linestyles='--'
    )
    axes[0].plot(
        0, 0, 'o', color='yellow', mec='black', mew=2, markersize=14, label='current (0, 0)'
    )
    ...
    axes[0].set_title(
        'Min tet volume vs. shifts in dz of corners 2 and 6\n(this cube only — neighbours ignored)'
    )
    axes[0].legend()
    ...
    axes[1].plot(0, 0, 'o', color='yellow', mec='black', mew=2, markersize=14, label='current')
    axes[1].set_xlabel('shift in dz(corner 6)')
    axes[1].set_ylabel('shift in dz(corner 2)')
    axes[1].set_title(
        f'Cube-feasibility region in (dz(2), dz(6)) space\n'
        f'{feasible_pct:.1f}% of this slice is locally feasible'
    )
    axes[1].legend()

    plt.suptitle(
        'The cube IS locally feasible if corners 2 and 6 separate along z\n'
        '(But moving them breaks ~5 neighbour cubes — Part X)',
        fontsize=13,
    )
    plt.tight_layout()
