...
def main():
    ...
    print(f'band z[44:76] {band.shape[1:]} n_neg={n0} n<thr={nb0} min_T={mt0:+.4f}', flush=True)
    ...
    for mode in ('windowed', 'fullgrid'):
        try:
            ...
            print(
                f'[GPU {mode}] n_neg {n0}->{n} n<thr->{nb} min_T={mt:+.5f} '
                f'feasible={n == 0} ({dt / 60:.1f} min)  '
                f'[CPU M10Tet baseline: 3.75 h to strict 0]',
                flush=True,
            )
