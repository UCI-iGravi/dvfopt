...
def main():
    ...
    print(
        f'crop {crop.shape[1:]} n_neg={int((mv <= 0).sum())} min_T={float(mv.min()):+.3f}',
        flush=True,
    )

    configs = [
        (
            'baseline   (inner=200, outer=60, rho_g=5)',
            dict(inner_maxiter=200, outer_max=60, rho_growth=5.0),
        ),
        (
            'fewer-inner (inner=50,  outer=60, rho_g=5)',
            dict(inner_maxiter=50, outer_max=60, rho_growth=5.0),
        ),
        (
            'fewer-inner (inner=100, outer=60, rho_g=5)',
            dict(inner_maxiter=100, outer_max=60, rho_growth=5.0),
        ),
        (
            'fast-rho    (inner=100, outer=60, rho_g=10)',
            dict(inner_maxiter=100, outer_max=60, rho_growth=10.0),
        ),
    ]
    for label, kw in configs:
        ...
        print(
            f'  {label}: wall={dt:5.1f}s  n_neg={n_neg:4d}  n<thr={n_bel:5d}  '
