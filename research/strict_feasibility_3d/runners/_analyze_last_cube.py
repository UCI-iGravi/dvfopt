...
def vols_for_diag(pos, di):
    s, e = _DIAGONALS[di]
    if di == 0:
        tets = _TET_VERTICES; signs = _TET_SIGN
    else:
...
def analyse_cube(phi, label, cz, cy, cx, phi_input=None):
    ...
    for (a, b) in edges_lattice:
        ...
        print(f'  edge ({a}, {b}): |pos[b]-pos[a]|={d_len:.4f}  '
    ...
    for di in range(4):
        ...
        s, e = _DIAGONALS[di]
        neg = (vols < 0).sum()
        print(f'  diag {(s,e)}: vols={[f"{v:+.5f}" for v in vols]}  '
    ...
    print(f'  min={grid_vals.min():+.5f}  max={grid_vals.max():+.5f}  '
    ...
    print(f'  det(J)={np.linalg.det(J_center):+.5f}  '
    ...
    for i in range(8):
        ...
        print(f'  corner {i} (u,v,w)=({ix},{iy},{iz}): '
              f'sigma=({sv_i[0]:.3f},{sv_i[1]:.3f},{sv_i[2]:.3f})  '
              f'det={det:+.5f}', flush=True)
...
def plot_cube_3d(pos, all_diag_vols, label, fname):
    ...
    for di in range(4):
        ...
        ax.scatter(pos[[s, e], 0], pos[[s, e], 1], pos[[s, e], 2],
                   c='yellow', s=120, edgecolor='black', zorder=20)
        n_neg = (all_diag_vols[di] < 0).sum()
        ax.set_title(f'Diagonal {(s,e)}  min={all_diag_vols[di].min():+.5f}  '
                     f'#neg={n_neg}')
    ...
    for di in range(4):
        s, e = _DIAGONALS[di]
        colors = ['red' if v < 0 else ('orange' if v < THRESHOLD else 'green')
                  for v in all_diag_vols[di]]
        ax6.bar(xs + di * width, all_diag_vols[di], width=width,
                label=f'diag {(s,e)}', edgecolor='black', linewidth=0.5)
...
def plot_neighbourhood(phi, cz, cy, cx, fname):
    ...
    for ax, ring in zip(axes, [0, 2, 5]):
        z0 = max(0, cz - ring); z1 = min(min_V.shape[0], cz + ring + 1)
        y0 = max(0, cy - ring); y1 = min(min_V.shape[1], cy + ring + 1)
        x0 = max(0, cx - ring); x1 = min(min_V.shape[2], cx + ring + 1)
        sub = min_V[z0:z1, y0:y1, x0:x1]
        # Project to y-x plane by taking min over z.
        proj = sub.min(axis=0)
        vmax = max(abs(proj.min()), abs(proj.max()), 1e-3)
        im = ax.imshow(proj, cmap='RdBu_r', vmin=-vmax, vmax=vmax,
                       origin='lower', extent=[x0, x1, y0, y1])
        ax.scatter([cx + 0.5], [cy + 0.5], c='yellow', s=120,
                    edgecolor='black', zorder=10, label='target')
        ax.set_title(f'min(tet_vol) projection, ring={ring}\n'
                     f'(z-min over z in [{z0}, {z1}])')
...
def main():
    ...
    print(f'BEST_1fold loaded: n_neg={int((V <= 0).sum())}  '
          f'n<0.01={int((V < 0.01 - 1e-5).sum())}  '
          f'min_T={float(V.min()):+.6f}', flush=True)

    # Analyse the fold cube.
    pos, diag_vols, grid_vals, J_c, sv = analyse_cube(
        cur, 'BEST_1fold', 1, 215, 220, phi_input)

    plot_cube_3d(pos, diag_vols, 'BEST_1fold cube (1,215,220)',
                 'last_fold_cube_3d.png')
    ...
    print('#'*70, flush=True)
    pos_in, diag_vols_in, _, _, sv_in = analyse_cube(
        phi_input, 'INPUT', 1, 215, 220)

    plot_cube_3d(pos_in, diag_vols_in, 'INPUT cube (1,215,220)',
                 'last_fold_cube_input.png')
    ...
    for i in range(8):
        d = pos_delta[i]
        m = np.linalg.norm(d)
        print(f'  corner {i}: delta=({d[0]:+.4f},{d[1]:+.4f},{d[2]:+.4f})  '
              f'|delta|={m:.4f}', flush=True)
