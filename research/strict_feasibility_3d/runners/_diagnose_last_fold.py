...
def main():
    ...
    print(f'Loaded: n_neg={int((V <= 0).sum())}  '
    ...
    for ci, (cz, cy, cx) in enumerate(cells):
        print(f'  cell {ci}: (z, y, x) = ({cz}, {cy}, {cx})  '
              f'min_T = {min_per_cell[cz, cy, cx]:+.6f}', flush=True)
    ...
    print(f'\n=== Cube ({cz}, {cy}, {cx}) under 4 diagonal triangulations ===',
          flush=True)
    best_diag = None
    best_min = -float('inf')
    for di in range(4):
        ...
        print(f'  diagonal {_DIAGONALS[di]}: tet vols = {vols.round(6).tolist()}  '
              f'min = {min_v:+.6f}', flush=True)
        if min_v > best_min:
            best_min = min_v
            best_diag = di
    print(f'\n  BEST diagonal: {_DIAGONALS[best_diag]}  min_T = {best_min:+.6f}',
          flush=True)
    if best_min > 0:
    ...
    for i in range(8):
        ...
        dy = cur[1, z, y, x]
        dx = cur[2, z, y, x]
        print(f'  corner {i} (lattice {z}, {y}, {x}): displacement '
    ...
    print('  Initial diagonal-0 tet volumes:', get_vols(x0, 0).round(6).tolist(),
          flush=True)
    res = minimize(
        objective, x0, method='SLSQP', constraints=cons,
        options={'maxiter': 200, 'ftol': 1e-9, 'disp': True},
    )
    print(f'  SLSQP success={res.success}  fun={res.fun:.6f}  '
          f'message={res.message}', flush=True)
    print(f'  Final diagonal-0 tet volumes: {get_vols(res.x, 0).round(6).tolist()}',
          flush=True)
