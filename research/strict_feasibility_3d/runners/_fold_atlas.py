...
def analyse_checkpoint(phi_path, phi_input, label):
    ...
    for i, (centroid, mems) in enumerate(zip(centroids, members)):
        print(f'    cluster {i}: centroid={centroid}, {len(mems)} cube(s)', flush=True)

    # Aggregate stats.
    print('\n  Aggregate cube properties:', flush=True)
    print(
        f'    crush_ratio   median={np.median([r["crush_ratio"] for r in rows]):.4f}  '
        f'min={min(r["crush_ratio"] for r in rows):.4f}  '
        f'max={max(r["crush_ratio"] for r in rows):.4f}',
        flush=True,
    )
    print(
        f'    sigma_3       median={np.median([r["svd"][2] for r in rows]):.4f}  '
        f'min={min(r["svd"][2] for r in rows):.4f}  '
        f'max={max(r["svd"][2] for r in rows):.4f}',
        flush=True,
    )
    print(
        f'    best_diag_min median={np.median([r["best_min"] for r in rows]):+.5f}  '
        f'#cubes-feasible-under-some-diag={sum(r["best_min"] > 0 for r in rows)}/{len(rows)}',
        flush=True,
    )
    print(
        f'    best_diag distribution: {[sum(r["best_di"] == d for r in rows) for d in range(4)]}',
        flush=True,
    )
...
def main():
    ...
    print('=' * 70, flush=True)
    print(
        f'{"Checkpoint":<25} {"#folds":>7} {"med crush":>10} {"med sigma_3":>12} {"any-diag-feas":>14}',
        flush=True,
    )
    print('-' * 70, flush=True)
    for label, (_, rows) in all_results.items():
        ...
        print(
            f'{label:<25} {len(rows):>7} {med_crush:>10.4f} {med_sig3:>12.4f} '
