...
def run_coupled(
    ...
    method='SLSQP',
    max_iter=300,
    feasibility_thr=None,
    relaxation_corners=None,
    relaxation_thr=None,
...
def main():
    ...
    def maybe_update(new_phi, label):
        ...
        if n < best_global_n_neg or (
            n == best_global_n_neg
            and b < int((six_tet_volumes_3d(best_phi) < THRESHOLD - 1e-5).sum())
        ):
            best_global_n_neg = n
    ...
    if res_a is not None:
        print(
            f'  k=2 SLSQP: success={res_a.success}  fun={res_a.fun:.4f}  '
            f'iter={res_a.nit}  wall={wall_a:.1f}s',
            flush=True,
        )
    if maybe_update(phi_a, 'A.1 k=2 SLSQP'):
        return
    ...
    if maybe_update(phi_a_recovered, 'A.2 k=2 + M10Tet@0.012'):
        return
    ...
    print(f'  M10Tet recovery wall={time.time() - t0:.1f}s', flush=True)
    if maybe_update(phi_a_r2, 'A.3 k=2 + M10Tet@0.010'):
        return
    ...
    if res_b is not None:
        print(
            f'  k=3 SLSQP: success={res_b.success}  fun={res_b.fun:.4f}  '
            f'iter={res_b.nit}  wall={wall_b:.1f}s  '
            f'#cubes={len(cubes_b)}  #DOF={3 * len(free_b)}',
            flush=True,
        )
    if maybe_update(phi_b, 'B.1 k=3 SLSQP'):
        return
    ...
    print(f'  M10Tet recovery wall={time.time() - t0:.1f}s', flush=True)
    if maybe_update(phi_b_r, 'B.2 k=3 + M10Tet@0.012'):
        return
    ...
    for seed in range(8):
        phi_c, res_c, wall_c, _, _ = run_coupled(
            cur, FOLD_CZ, FOLD_CY, FOLD_CX, k_ring=2, init_perturbation=(seed, 0.01)
        )
        ok = res_c is not None and res_c.success
        n_c, _, mn = report(phi_c, f'  C seed={seed} (success={ok})', phi_input)
        if maybe_update(phi_c, f'C seed={seed}'):
            return
    ...
    try:
        phi_d, res_d, wall_d, _, _ = run_coupled(cur, FOLD_CZ, FOLD_CY, FOLD_CX, k_ring=2)
        if res_d is not None:
            print(
                f'  D k=2 (thr=1e-3): success={res_d.success}  fun={res_d.fun:.4f}  '
                f'wall={wall_d:.1f}s',
                flush=True,
            )
        if maybe_update(phi_d, 'D.1 k=2 thr=1e-3'):
            return
        ...
        print(f'  M10Tet recovery wall={time.time() - t0:.1f}s', flush=True)
        if maybe_update(phi_d_r, 'D.2 k=2 thr=1e-3 + M10Tet'):
            return
    finally:
    ...
    phi_e, res_e, wall_e, cubes_e, free_e = run_coupled(
        cur, FOLD_CZ, FOLD_CY, FOLD_CX, k_ring=4, method='trust-constr', max_iter=200
    )
    if res_e is not None:
        print(
            f'  k=4 trust-constr: status={res_e.status} '
            f'fun={res_e.fun:.4f}  iter={res_e.nit}  '
            f'wall={wall_e:.1f}s  #cubes={len(cubes_e)}  '
            f'#DOF={3 * len(free_e)}',
            flush=True,
        )
    if maybe_update(phi_e, 'E.1 k=4 trust-constr'):
        return
    ...
    print(f'  M10Tet recovery wall={time.time() - t0:.1f}s', flush=True)
    if maybe_update(phi_e_r, 'E.2 k=4 + M10Tet@0.012'):
        return

    # Save final best.
    print(f'\n=== Final ===\n  best n_neg={best_global_n_neg} via {best_method}', flush=True)
