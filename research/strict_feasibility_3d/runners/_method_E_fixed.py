...
def run_trust_constr(phi, k_ring=4, max_iter=200):
    ...
    print(
        f'  cubes={len(cubes)}  DOF={3 * len(free_corners)}  constraints={6 * len(cubes)}',
        flush=True,
    )
    ...
    res = minimize(
        ...
        constraints=nlc,
        method='trust-constr',
        options={'maxiter': max_iter, 'xtol': 1e-8, 'gtol': 1e-6, 'disp': True, 'verbose': 2},
    )
    wall = time.time() - t0
    print(
        f'  trust-constr: status={res.status}  fun={res.fun:.4f}  '
...
def main():
    ...
    print('\n' + '=' * 70, flush=True)
    print('METHOD E (FIXED): coupled k=4 trust-constr + M10Tet @ 0.012', flush=True)
    print('=' * 70, flush=True)
