...
def main():
    ...
    print(
        f'  largest sv:    min={svd_specs[:, 0].min():.3f}  median={float(np.median(svd_specs[:, 0])):.3f}  max={svd_specs[:, 0].max():.3f}',
        flush=True,
    )
    print(
        f'  middle sv:     min={svd_specs[:, 1].min():.3f}  median={float(np.median(svd_specs[:, 1])):.3f}  max={svd_specs[:, 1].max():.3f}',
        flush=True,
    )
    print(
        f'  smallest sv:   min={svd_specs[:, 2].min():.3f}  median={float(np.median(svd_specs[:, 2])):.3f}  max={svd_specs[:, 2].max():.3f}',
        flush=True,
    )
    print(
        f'  aspect ratio (sigmamax/sigmamin): min={aspect_ratio.min():.2f}  median={float(np.median(aspect_ratio)):.2f}  max={aspect_ratio.max():.2f}',
        flush=True,
    )
    ...
    print(
        f'  det(J) at center:  min={det_J_centroid.min():.3f}  median={float(np.median(det_J_centroid)):.3f}  max={det_J_centroid.max():.3f}',
        flush=True,
    )
    print(
        f'  det(J) min (5x5x5 grid):  min={det_J_min.min():.3f}  median={float(np.median(det_J_min)):.3f}  max={det_J_min.max():.3f}',
        flush=True,
    )
    print('  fraction of internal grid with det(J)<0:', flush=True)
    print(
        f'    min={det_J_frac_neg.min():.2%}  median={float(np.median(det_J_frac_neg)):.2%}  max={det_J_frac_neg.max():.2%}',
        flush=True,
    )

    print('\n=== Jacobian eigenvalues at center ===', flush=True)
    print(
        f'  smallest real eigenvalue:  min={jac_eigvals_min.min():.3f}  median={float(np.median(jac_eigvals_min)):.3f}  max={jac_eigvals_min.max():.3f}',
        flush=True,
    )
    ...
    print(
        f'  centroid:  ({float(z_arr.mean()):.1f}, {float(y_arr.mean()):.1f}, {float(x_arr.mean()):.1f})',
        flush=True,
    )
    ...
    print(
        f'  Unfixable: sigma_min median={float(np.median(svd_specs[:, 2])):.3f}  det(J)_min median={float(np.median(det_J_min)):.3f}',
        flush=True,
    )
    print(
        f'  Fixable:   sigma_min median={float(np.median(fix_svd_min)):.3f}  det(J)_min median={float(np.median(fix_det_min)):.3f}',
        flush=True,
    )
    ...
    axes[0, 0].hist(svd_specs[:, 0], bins=30, color='C0', alpha=0.7)
    axes[0, 0].set_title('Largest singular value (sigma_1)')
    axes[0, 1].hist(svd_specs[:, 1], bins=30, color='C1', alpha=0.7)
    axes[0, 1].set_title('Middle singular value (sigma_2)')
    axes[0, 2].hist(svd_specs[:, 2], bins=30, color='C2', alpha=0.7)
    axes[0, 2].set_title('Smallest singular value (sigma_3)')
    axes[1, 0].hist(det_J_min, bins=30, color='C3', alpha=0.7)
    axes[1, 0].set_title('min det(J) inside cube (continuous)')
    axes[1, 0].axvline(0, color='k', linewidth=0.5)
    axes[1, 1].hist(det_J_frac_neg, bins=30, color='C4', alpha=0.7)
    axes[1, 1].set_title('Fraction of cube interior with det(J)<0')
    axes[1, 2].hist(aspect_ratio, bins=30, color='C5', alpha=0.7)
    axes[1, 2].set_title('Aspect ratio (sigma_1/sigma_3)')
    for ax in axes.flat:
