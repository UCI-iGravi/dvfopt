...
def main():
    ...
    # ============================================================
    print('\n' + '=' * 70, flush=True)
    print('METHOD C: Multi-restart k=2 SLSQP (8 seeds) + M10Tet @ 0.012', flush=True)
    print('=' * 70, flush=True)
    best_c_n_neg = 999
    best_c_phi = None
    best_c_seed = None
    for seed in range(8):
        ...
        if n_c < best_c_n_neg:
            ...
            best_c_seed = seed
    print(f'\n  Best multi-restart SLSQP: seed={best_c_seed} n_neg={best_c_n_neg}', flush=True)
    ...
    # ============================================================
    print('\n' + '=' * 70, flush=True)
    print('METHOD D: k=2 SLSQP with FEASIBILITY_THR=1e-3 (Lagrangian) + M10Tet', flush=True)
    print('=' * 70, flush=True)
    ...
    try:
        ...
        if res_d is not None:
            print(
                f'  D k=2 (thr=1e-3): success={res_d.success}  '
                f'fun={res_d.fun:.4f}  wall={wall_d:.1f}s',
                flush=True,
            )
    ...
    # ============================================================
    print('\n' + '=' * 70, flush=True)
    print('METHOD E: coupled k=4 trust-constr + M10Tet @ 0.012 recovery', flush=True)
    print('=' * 70, flush=True)
    ...
    if res_e is not None:
        print(
            f'  k=4 trust-constr: status={res_e.status} fun={res_e.fun:.4f}  '
            f'iter={res_e.nit}  wall={wall_e:.1f}s  '
            f'cubes={len(cubes_e)}  DOF={3 * len(free_e)}',
            flush=True,
        )
    ...
    print('=' * 70, flush=True)
    print(f'{"Method":<40} {"n_neg":>6} {"n<0.01":>8} {"wall (s)":>10}', flush=True)
    print('-' * 68, flush=True)
    for label, n, b, w in results:
        marker = '  *** STRICT ***' if n == 0 and b == 0 else ''
        print(f'{label:<40} {n:>6} {b:>8} {w:>10.1f}{marker}', flush=True)
