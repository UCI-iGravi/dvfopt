...
def iterate_slsqp_per_fold(phi, label='', phi_input=None):
    ...
    for iter_i in range(MAX_ITER_SLSQP_LOOP):
        ...
        if result is None:
            print(f'  [{label}] iter {iter_i}: no folds remain (n_neg=0)', flush=True)
            break
        loc, min_T = result
        cz, cy, cx = loc
        ...
        if not (
            ...
            print(
                f'  [{label}] iter {iter_i}: worst fold at ({cz},{cy},{cx}) '
        ...
        print(
            f'  [{label}] iter {iter_i}: worst fold at ({cz},{cy},{cx}) min={min_T:+.6f}',
            flush=True,
        )
        ...
        if res is None or not res.success:
            print(f'  [{label}] iter {iter_i}: SLSQP did not converge; stopping', flush=True)
            break
        ...
        if n_after > n_before + 3:
            print(
                f'  [{label}] iter {iter_i}: SLSQP increased global n_neg '
        ...
        cur = new
        if n_after == 0:
            print(f'  [{label}] iter {iter_i}: STRICT global feasibility achieved', flush=True)
            break
...
def main():
    ...
    # ============================================================
    print('\n' + '=' * 70, flush=True)
    print('VARIANT A: M14Tet -> per-fold k=2 SLSQP @ thr=1e-3 -> M10Tet recover', flush=True)
    print('=' * 70, flush=True)
    t0 = time.time()
    ...
    phi_a2, iter_a, wall_a2 = iterate_slsqp_per_fold(phi_a1, 'VARIANT-A', phi_input)
    ...
    print(f'\n  VARIANT A FINAL: n_neg={n_a}  n<0.01={b_a}  total wall={total_a:.1f}s', flush=True)
    if n_a == 0 and b_a == 0:
    ...
    # ============================================================
    print('\n' + '=' * 70, flush=True)
    print('VARIANT B: M10Tet -> per-fold k=2 SLSQP @ thr=1e-3 -> M10Tet recover', flush=True)
    print('=' * 70, flush=True)
    t0 = time.time()
    ...
    phi_b2, iter_b, wall_b2 = iterate_slsqp_per_fold(phi_b1, 'VARIANT-B', phi_input)
    ...
    print(f'\n  VARIANT B FINAL: n_neg={n_b}  n<0.01={b_b}  total wall={total_b:.1f}s', flush=True)
    if n_b == 0 and b_b == 0:
    ...
    print('=' * 70, flush=True)
    print(f'{"Variant":<10} {"n_neg":>6} {"n<0.01":>8} {"wall (s)":>12}', flush=True)
    print('-' * 38, flush=True)
