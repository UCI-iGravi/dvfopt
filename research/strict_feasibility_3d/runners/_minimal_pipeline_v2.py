...
def main():
    ...
    print('\n=== Re-cluster big clusters (radius>4) at radius=1 ===', flush=True)
    ...
    for i, (c, mem, r) in enumerate(zip(centroids, members, radii)):
        ...
        print(
            f'  big cluster {i} (size={len(mem)}, r={r}) -> {len(sub_centroids)} sub-clusters',
            flush=True,
        )
        for j, (sc, sm, sr) in enumerate(zip(sub_centroids, sub_members, sub_radii)):
            print(f'    sub {j}: centroid={sc}, size={len(sm)}, r={sr}', flush=True)
            refined_clusters.append((sc, sm, sr))
    print(f'\n  total refined (sub-)clusters: {len(refined_clusters)}', flush=True)
    ...
    for i, (c, mem, r) in enumerate(
        ...
        if delta <= 0:
            ...
            rejected += 1
    print(f'\n  Accepted: {accepted}, rejected: {rejected}, skipped: {skipped}', flush=True)
