...
def main():
    ...
    for z in zs:
        ...
        for sched in ('subround', 'continuous'):
            t0 = time.time()
            out, info = cluster_slp_iter(
                sl, threshold=THR, max_outer_iters=6, n_workers=NW, scheduler=sched
            )
            dt = time.time() - t0
            res[sched] = (
                dt,
                _n_neg_2tri(out),
                float(np.abs(out - sl).sum()),
                info.get('total_cluster_solves'),
            )
            print(
                f'  {sched:>10}: wall={dt:6.1f}s  n_neg={res[sched][1]:4d}  '
                f'L1={res[sched][2]:10.1f}  cluster_solves={res[sched][3]}',
                flush=True,
            )
        sub, con = res['subround'], res['continuous']
        print(
            f'  -> continuous speedup {sub[0] / con[0]:.2f}x  '
            f'L1 delta {(con[2] - sub[2]) / sub[2] * 100:+.2f}%  '
            f'feasible both={sub[1] == 0 and con[1] == 0}',
            flush=True,
        )
