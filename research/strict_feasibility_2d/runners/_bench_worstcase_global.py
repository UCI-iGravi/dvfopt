...
def main():
    ...
    print(f'[clustered] wall={wc:.1f}s  n_neg={_n_neg(out_c)}  L1={l1c:.1f}',
          flush=True)
    ...
    print(f'[global]    wall={wg:.1f}s  n_neg={_n_neg(out_g)}  L1={l1g:.1f}',
          flush=True)

    print(f'\nWORST-CASE SAVING: {wg - wc:.0f}s  ({wg/wc:.1f}x faster)  '
          f'L1 gap clustered vs global = {(l1c-l1g)/l1g*100:+.1f}%', flush=True)
