...
def main():
    ...
    for z, half in [(300, 32), (12, 40)]:
        ...
        a = tri_areas_flat(np.concatenate([crop[0].ravel(), crop[1].ravel()]), *crop.shape[1:])
        n0 = int((a <= 0).sum())
        print(
            f'\n=== z={z} crop {crop.shape[1:]} '
            f'({2 * crop.shape[1] * crop.shape[2]} vars) input n_neg={n0} '
            f'min_T={float(a.min()):+.3f} ===',
            flush=True,
        )
        ...
        print(
            f'  L-BFGS-B: wall={wb:6.2f}s  inner~{inner_b:>5}(partial log)  '
            f'n_neg={nn} n<thr={nb} min_T={mt:+.5f} L1={l1:.2f}',
            flush=True,
        )
        ...
        print(
            f'  GN      : wall={wg:6.2f}s  inner={info_g["total_inner"]:>5}       '
            f'  n_neg={nn} n<thr={nb} min_T={mt:+.5f} L1={l1:.2f}',
            flush=True,
        )
