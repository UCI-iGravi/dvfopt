"""Bench the SHIFTED-barrier GPU untangler (starts from phi_in).

Target: feasible by construction (like the plain barrier) AND low L1 (like
the from-phi_in penalty basin). Starts at phi_in with per-triangle slacks,
anneals them to 0. Compares folds/L1/wall vs champion on mild + dense +
worst slices. Verified against numpy tri_areas_flat.
"""
...
def main():
    ...
    for z, tag in ((300, 'mild'), (13, 'dense'), (12, 'worst')):
        ...
        t0 = time.time()
        g, info = gpu_shifted_barrier_untangle_2d(sl, threshold=THR, verbose=1)
        wg = time.time() - t0
        print(f'  GPU-shifted       : wall={wg:6.1f}s  folds(np)={_nneg(g)}  '
