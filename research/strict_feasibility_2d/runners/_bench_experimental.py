...
def main():
    ...
    # ---- (1) Seed-cost sweep ----
    print('=== (1) SEED-COST SWEEP (cluster_slp_iter, n_workers=16, continuous) ===',
          flush=True)
    for z in [300, 450]:
        ...
        for seed in ['harmonic', 'm10', 'm14_quick', 'm14_fast']:
            ...
            try:
                out, _ = cluster_slp_iter(
                    sl, threshold=THR, max_outer_iters=6, n_workers=16,
                    scheduler='continuous', inner_seed=seed,
                )
                dt = time.time() - t0
                print(f'{seed:>10} | {dt:>8.1f} | {_n_neg(out):>5} | '
                      f'{float(np.abs(out - sl).sum()):>10.1f}', flush=True)
            except Exception as e:
                print(f'{seed:>10} | FAILED: {type(e).__name__}: {e}', flush=True)

    # ---- (2) L1-optimality gap: clustered vs global SLP ----
    print('\n=== (2) L1-OPTIMALITY GAP (clustered auto_slp vs global slp_iter) ===',
          flush=True)
    for z in [450]:  # global on one slice (whole-slice LP is slow)
        sl = raw[1:3, z].astype(np.float64)
        t0 = time.time()
        out_c, _ = cluster_slp_iter(sl, threshold=THR, max_outer_iters=6,
        ...
        print(f' z={z}: clustered L1={l1c:.1f} ({wc:.1f}s, n_neg={_n_neg(out_c)})  '
              f'global L1={l1g:.1f} ({wg:.1f}s, n_neg={_n_neg(out_g)})  '
              f'gap={(l1c-l1g)/l1g*100:+.2f}%', flush=True)
