...
def main():
    ...
    print(f'=== (A) merge_dilation sweep (z={z}, target global L1={GLOBAL_L1}) ===', flush=True)
    print(f'{"merge_dil":>9} | {"wall(s)":>8} | {"n_neg":>5} | {"L1":>9} | {"gap%":>7}', flush=True)
    for md in [1, 2, 4, 8]:
        t0 = time.time()
        ...
        print(
            f'{md:>9} | {dt:>8.1f} | {_n_neg(out):>5} | {l1:>9.1f} | '
            f'{(l1 - GLOBAL_L1) / GLOBAL_L1 * 100:>+6.1f}%',
            flush=True,
        )

    print(f'\n=== (B) warm-started global L1 polish (z={z}) ===', flush=True)
    t0 = time.time()
    base, _ = cluster_slp_iter(
        sl, threshold=THR, max_outer_iters=6, n_workers=16, scheduler='continuous'
    )
    t_cluster = time.time() - t0
    l1_base = float(np.abs(base - sl).sum())
    print(f'  cluster: L1={l1_base:.1f} ({t_cluster:.1f}s, n_neg={_n_neg(base)})', flush=True)
    for nsteps in [1, 3]:
        t0 = time.time()
        ...
        print(
            f'  +{nsteps} global polish step(s): L1={l1:.1f} '
            f'(+{dt:.1f}s, n_neg={_n_neg(pol)}) '
            f'gap={(l1 - GLOBAL_L1) / GLOBAL_L1 * 100:+.1f}%',
            flush=True,
        )
