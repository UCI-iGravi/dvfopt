...
- **phi flattening — TWO conventions exist, do not cross-mix them:**
  ...
  - The split is **not** "simplex (2D)/simplex (3D) vs Jdet" — it is `Constraint.pack`. A flat phi vector from one pack CANNOT be passed to a helper of the other without channel-swapping; a helper that sees both (`dvfopt/core/marching/*`) asserts on the pack lengths. Helpers in `dvfopt/core/primitives/tri.py` and `anchor_term` in `dvfopt/objectives.py` are the single sources of truth — reuse them rather than re-deriving partials. See [ARCHITECTURE.md](ARCHITECTURE.md).
...
- `dvfopt/` — the installable package; **one package, no sibling top-level packages** since 0.5.0. `core/` is method-first: `primitives/`, `nmvf/`, `barrier/`, `slsqp_windowed/`, `slsqp_fullgrid/`, `schwarz/`, `wallbreakers/`, `slp/`, `marching/`. Alongside `core/`: `jacobian/`, `dvf/`, `viz/`, `io/`, `utils/`, plus the absorbed `laplacian/` and `testdata/`.
