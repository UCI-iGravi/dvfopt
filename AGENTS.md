...
- **phi flattening — TWO conventions exist, do not cross-mix them:**
  ...
  - The split is **not** "2-tri/6-tet vs Jdet" — it is `Constraint.pack`. A flat phi vector from one pack CANNOT be passed to a helper of the other without channel-swapping; a helper that sees both (`dvfopt/core/marching/*`) asserts on the pack lengths. Helpers in `dvfopt/core/primitives/tri.py` and `anchor_term` in `dvfopt/objectives.py` are the single sources of truth — reuse them rather than re-deriving partials. See [ARCHITECTURE.md](ARCHITECTURE.md).
