...
def run_one(name, full_coverage):
    ...
    try:
        phi_out = _silent(iterative_2d_tri_barrier, phi_init.copy(),
                          threshold=0.01, max_minimize_iter=300, verbose=0,
                          full_coverage=full_coverage)
    except Exception as exc:
        return dict(case=name, scheme='full_coverage' if full_coverage else 'standard',
                    error=f'{type(exc).__name__}: {exc}', wall_s=time.perf_counter() - t0)
    wall = time.perf_counter() - t0
    final = _stats(phi_out)
    l2 = float(np.linalg.norm(phi_out - phi_init))
    return dict(
        case=name, scheme='full_coverage' if full_coverage else 'standard',
        wall_s=wall,
...
def main():
    rows = []
    header_fmt = ("{case:<32} {scheme:<14} {wall_s:>7} "
                  "{init_tri_neg:>6} {final_tri_neg:>7} {final_tri_min:>10} "
                  "{init_patches_min:>10} {final_patches_min:>10} "
                  "{l2:>7} {feas_std:>9} {feas_full:>9}")
    print(header_fmt.format(
        case='case', scheme='scheme', wall_s='wall_s',
        init_tri_neg='init_n', final_tri_neg='final_n', final_tri_min='final_min',
        init_patches_min='init_p', final_patches_min='final_p',
        l2='L2', feas_std='feas_std', feas_full='feas_full'))
    print('-' * 145)
    ...
    cols = ['case', 'scheme', 'wall_s',
            'init_tri_neg', 'init_tri_min', 'init_patches_min',
            'final_tri_neg', 'final_tri_min', 'final_patches_min',
            'final_full_neg', 'l2', 'feas_standard', 'feas_full']
    ...
    for scheme in ('standard', 'full_coverage'):
        ...
        print(f"{scheme:<14} {avg_wall:>11.2f} {fs*100:>14.0f}% {ff*100:>15.0f}% {avg_l2:>9.2f}")
