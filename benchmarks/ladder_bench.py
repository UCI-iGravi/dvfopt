...
def rung(phi, phi0, ctype, box, target, thr, a, log):
    ...
    if a.hess == "laplacian":
        # elastic prior over the patch grid: H = 2*(eps*I + lam*L), L = 4-neighbour
        # graph Laplacian per channel (block-diag over the constraint's pack order).
        # Smooth coordinated steps become cheap; spiky single-node moves expensive.
        ...
        for di, dj in ((0, 1), (1, 0)):
            a_ = idx[: ph - di, : pw - dj].ravel()
            b_ = idx[di:, dj:].ravel()
            rr += [a_, b_]
            cc += [b_, a_]
        rr, cc = np.concatenate(rr), np.concatenate(cc)
        A = sparse.csr_matrix((np.ones(rr.size), (rr, cc)), shape=(n, n))
        L = sparse.diags(np.asarray(A.sum(axis=1)).ravel()) - A
        H1 = 2.0 * (a.hess_eps * sparse.eye(n) + a.hess_lam * L)
        Hfull = sparse.block_diag([H1, H1], format="csc")

        def hess(_f, _H=Hfull):
            return _H
    ...
    x, nit, _ok = isqp_solve(
        ...
        monotone_accept=a.monotone_accept,
    )
...
def main():
    ...
    ap.add_argument(
        "--hess", default="diag", choices=["diag", "laplacian"], help="QP curvature model"
    )
    ap.add_argument("--hess-lam", type=float, default=1.0, help="Laplacian weight (grid units)")
    ap.add_argument(
        "--hess-eps", type=float, default=0.1, help="identity floor for the Laplacian prior"
    )
    ap.add_argument(
        "--monotone-accept",
        action="store_true",
        help="reject steps where any row worsens (nonlinear)",
    )
