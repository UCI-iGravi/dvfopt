...
def run_barrier_tri(deformation):
    ...
    return _quiet_call(
        iterative_2d_tri_barrier, phi2, threshold=0.01, max_minimize_iter=300, verbose=0
    )


def run_dvfopt_auto_jdet(deformation):
    cfg = DVFoptConfig(solver='auto', constraint='jdet', threshold=0.01, verbose=0)
...
def run_one(slice_path: str, method_name: str, method_fn):
    ...
    try:
        ...
        return dict(
            method=method_name,
            wall_s=wall,
            init_jdet_neg=init['jdet_neg'],
            init_jdet_min=init['jdet_min'],
            init_tri_neg=init['tri_neg'],
            init_tri_min=init['tri_min'],
            final_jdet_neg=-1,
            final_jdet_min=float('nan'),
            final_tri_neg=-1,
            final_tri_min=float('nan'),
            l2=float('nan'),
            jdet_feasible=False,
            tri_feasible=False,
...
def main():
    ...
    for slice_name, slice_path in SLICES:
        for method_name, method_fn in METHODS:
            ...
            print(
                f"{slice_name:<10} {method_name:<22} {row['wall_s']:>7.2f} "
                f"{row['final_jdet_neg']:>10d} {row['final_jdet_min']:>+10.4f} "
                f"{row['final_tri_neg']:>9d} {row['final_tri_min']:>+10.4f} "
                f"{row['l2']:>8.2f} {row['jdet_feasible']!s:>7} "
                f"{row['tri_feasible']!s:>7}"
            )
        print()
