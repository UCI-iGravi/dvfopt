...
def run_one(label, fn, phi_in):
    ...
    print(f'  wall={wall:.2f}s  n_neg={n_neg}  min_T={min_T:+.5f}  '
          f'L1={l1:.1f}  L2={l2:.2f}  feasible={feasible}', flush=True)
    return dict(method=label, wall_s=wall, n_neg=n_neg, min_T=min_T,
                L1=l1, L2=l2, feasible=feasible)
...
def main():
    ...
    print(f'  {H*W:,} corners, {2*H*W:,} variables, '
          f'{2*(H-1)*(W-1):,} constraints', flush=True)
    print(f'  init n_neg={init_n_neg:,}  init min_T={init_min:+.3f}',
          flush=True)
    print(flush=True)

    methods = [
        ('barrier_l1', lambda p: _silent(
            iterative_2d_tri_barrier, p,
            threshold=THRESHOLD, margin=1e-3,
            max_minimize_iter=500,
            anchor='l1', eps_l1=1e-4, verbose=0)),
        ('m10_default', lambda p: _silent(
            iterative_2d_tri_harmonic_polished, p,
            threshold=THRESHOLD, margin=1e-3,
            anchor='l1', eps_l1=1e-4,
            max_grow_iters=8,
            time_budget_s=900.0, verbose=0)),
        ('m10_grow20', lambda p: _silent(
            iterative_2d_tri_harmonic_polished, p,
            threshold=THRESHOLD, margin=1e-3,
            anchor='l1', eps_l1=1e-4,
            max_grow_iters=20,
            time_budget_s=900.0, verbose=0)),
        ('m14_default_l2', lambda p: _silent(
            iterative_2d_tri_refine_repair, p,
            threshold=THRESHOLD, margin=1e-3,
            anchor='l2', eps_l1=1e-4,
            max_grow_iters=8,
            time_budget_s=900.0, verbose=0)),
        ('m14_l1_grow20', lambda p: _silent(
            iterative_2d_tri_refine_repair, p,
            threshold=THRESHOLD, margin=1e-3,
            anchor='l1', eps_l1=1e-4,
            max_grow_iters=20,
