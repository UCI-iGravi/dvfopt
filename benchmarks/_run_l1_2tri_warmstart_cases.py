...
def solve_slsqp_windowed(phi_init):
    ...
    out = _silent(
        iterative_serial, deformation,
        verbose=0, threshold=THRESHOLD,
        max_iterations=80, max_minimize_iter=120,
...
def solve_slsqp_fullgrid(phi_init, anchor):
    ...
    return _silent(
        iterative_2d_tri_slsqp, phi_init.copy(),
        threshold=THRESHOLD,
        max_iter=80, warm_max_iter=1200,
        anchor=anchor, eps_l1=EPS_L1,
        full_coverage=True,
        verbose=0,
    )
...
def run_one(case_key, phi_init, method_label, fn):
    ...
    try:
        ...
        return dict(
            case=case_key, method=method_label,
            shape=phi_init.shape[1:],
            wall_s=time.perf_counter() - t0,
            init_cell_neg=init['cell_neg'], init_cell_min=init['cell_min'],
            init_patches_min=init['patches_min'],
            final_cell_neg=-1, final_cell_min=float('nan'),
            final_patches_min=float('nan'),
            final_full_neg=-1, final_full_min=float('nan'),
    ...
    return dict(
        ...
        init_cell_neg=init['cell_neg'], init_cell_min=init['cell_min'],
        init_patches_min=init['patches_min'],
        final_cell_neg=final['cell_neg'], final_cell_min=final['cell_min'],
        final_patches_min=final['patches_min'],
        final_full_neg=final['full_neg'], final_full_min=final['full_min'],
        l1=_l1(phi_out, phi_init),
        l2=_l2(phi_out, phi_init),
        feasible=(final['full_neg'] == 0
                  and final['full_min'] >= 0.01 - 1e-5),
        error='',
    )
...
def main():
    rows = []
    hfmt = ("{case:<22} {shape:<7} {method:<22} {wall_s:>6} "
            "{cell_neg_f:>8} {cell_min_f:>10} {p_min_f:>10} "
            "{l1:>8} {l2:>8} {feas:>6}")
    print(hfmt.format(
        case='case', shape='shape', method='method', wall_s='wall',
    ...
    cols = ['case', 'method', 'wall_s',
            'init_cell_neg', 'init_cell_min', 'init_patches_min',
            'final_cell_neg', 'final_cell_min', 'final_patches_min',
            'final_full_neg', 'final_full_min', 'l1', 'l2', 'feasible']
