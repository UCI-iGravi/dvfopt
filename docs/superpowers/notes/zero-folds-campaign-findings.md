...
## 14. The pin chain: 3D folds in Laplacian fields are pin contradictions, readable from the field

Section 13 found that the dense 3D-windowed-engine residual at z[0,24) of B0039's full-volume
route was upstream data, not an engine limit (13.6). This section is that follow-up
(2026-09-22 … 09-25): a DVF-only preprocessing step, run in front of the 2D + 2.5D pipeline
instead of the 3D engine, that reads a Laplacian field's own contradictions out of the field and
removes them before any solver runs. Per the standing scope rule
([[scope-dvf-only-never-touch-correspondences]]): the DVF is the only thing edited; correspondence
files are read only to compute a residual for reporting, never to build or filter the field.
Artefacts: `notebooks/experiments/source_space_pin_probe.ipynb` (generated by
`_build_source_space_probe_nb.py`, run as a scratch script — no nbconvert installed) and
`benchmarks/output/probe_source_space/` (gitignored: `run.log`, `full/`, `full_v2/`, `full_v3/`,
`full_v4/`, `cohort/`, `slices2d/`, `lipschitz*.{py,log,json}`, `results_B_all.json`,
`full_chain*.py`). The reusable module is `dvfopt/dvf/pins.py` (untracked as of this section —
14.9).

### 14.1 The mechanism

A field built by Laplacian interpolation is discrete-harmonic everywhere except at its Dirichlet
pins: the 6-neighbour graph Laplacian applied to the field (not solved against it) is ~0 at every
free voxel and spikes at the pins. `dvfopt/dvf/pins.py` reads this back with no correspondence
file in the loop: `source_map`/`source_strength` compute that stencil per channel,
`detect_pins(phi, tau)` thresholds it, `violating_pairs`/`violating_pairs_pruned` find the pairs
of detected pins whose displacements disagree faster than the geometry allows
(`|Δd(p) − Δd(q)| > c|p − q|` within a radius — they would have to cross), and `greedy_cover`
removes a small vertex-cover set (bucket-queue, O(m + n)) that leaves no violating pair.

On the probe's own re-solve (`B_all`, `rtol` 1e-4, `results_B_all.json`): `tau=0.1` recovers
8,379 of 8,381 true pins with precision 1.0 — the pins sit 1.5 decades above the CG residual. On
the **production field** (`rtol` 1e-2) the same threshold is unusable: 101,610 detected against
8,381 true pins, precision 0.082 — the CG residual noise floor sits at the pin scale, so a
production field must be read after a tighter solve, or with a per-brain threshold (14.6).
Otsu thresholding failed on both fields; a fixed `tau` is what works.

The correspondence files are read exactly once per case, after the chain runs, to compute a
landmark residual (median px distance, % within 10 px) for the fidelity column — never to build,
filter or seed the field.

### 14.2 The slab probe (B0039 z[0,40), folds counted at z<24)

`source_space_pin_probe.ipynb`, three tests on the calibration field `B_all`:

- **Test 1** confirms the mechanism only on a tightly solved field (above).
- **Test 2**: the pairwise cover on the DVF-read 8,379 pins finds 870,618 violating pairs and
  drops 6,816 (keeps 1,563) — 96.4% of the correspondence-side probe's 5,934 drops recovered,
  Jaccard 0.81 against that independent (correspondence-based) cover.
- **Test 3**: a **global** harmonic re-fill (kept sources + a far-plane Dirichlet condition) takes
  73,012 folds (min −300.17) → **1,263** (min −1.22) — better than the correspondence-side probe's
  1,635 (min −1.06) — at a landmark residual median 3.42 px, 86.1% within 10 px. It moves 92.7% of
  voxels (mean 2.86 px, max 211.05 px): not a no-damage edit. **Local** re-fill fails outright —
  radius 2/4/8 leaves 64,316 / 58,352 / 46,757 folds (moved fraction 0.7% / 1.6% / 3.8%): the
  removed pins' influence falls off as 1/r, so only a global solve reaches the far side of the
  volume the contradiction actually corrupted.

### 14.3 The chain and its stages (v1 → v5)

The chain is: (tight re-solve, if the field is too loosely solved to read) → pin read → pairwise
drop → global harmonic re-fill → 2D windowed engine per slice → 2.5D marching
(`correct_dvf_25d`). Five iterations cut the wall from 5.5 h to well under 90 min without changing
the certificate.

- **v1** (`full/`, `full_chain.py`): tight re-solve at coarse `tau=0.7` (424,682 pins, CG 46 min)
  → fine read of the same 424,682 pins → 10.9M violating pairs → greedy cover in pure Python
  (`max(adj, key=len)`, O(n²)) — **2.8 h**, dropping 274,986 (65%) and keeping 149,696 → re-fill CG
  71 min: 967,234 folds (floor 830,174, min −312.38) → 64,879 (floor 42,673, min −2.78) → 2D engine
  528/528 slices in 5 min (median 1.4 s/slice, max 10 s — the raw median was 128–281 s, hours on
  the hard slices) → 2.5D in 14 min, sweep only, no mop. Result: feasible, 0 folds, 0 best-diagonal
  floor, min +0.0101; landmark residual median 1.03 px, 98.8% within 10 px; L2 move from raw
  17,848.8. Total ≈ 5.5 h, of which 3 h was the un-vectorised cover.
- **v2** (`full_v2/`, `full_chain_v2.py`): identical certificate (0/0/+0.0101, residual 1.02 px /
  98.7%) in **88 min** (`total_wall_s` 5,259 s). Levers: (1) no tight re-solve needed — the coarse
  `tau=0.7` read on the RAW field is the same 424,682-pin set with the same 10,927,126 violating
  pairs; (2) `violating_pairs` (chunked `sparse_distance_matrix`) + the bucket-queue `greedy_cover`
  — 12 min vs 2.8 h, the old cover was the O(n²) bottleneck; (3) re-fill CG warm-started at `x0 =
  raw` — 40 min (769/715 iterations) vs 71 min cold.
- **v3** (`full_v3/`): `violating_pairs(n_workers=4)` on a process pool — pair test 12 → 4 min.
  **Refuted here**: coarse-to-fine warm start for the re-fill CG only cut iterations 769 → 640
  (−17%), not enough to keep; running the re-fill's two channels as parallel processes was a
  **loss** (5.4 s/iteration vs 1.7 s serial) — the 77M-DOF sparse matvec is memory-bandwidth-bound,
  so channels must stay serial. `full_v3/report.json`'s own 2.5D wall (1,789 s) ran contended.
- **v4** (`full_v4/`): AMG is the re-fill lever. `cg_bench.json`: Jacobi-PCG 769 iterations / 2,368
  s (rel. resid 1.0e-4) vs smoothed-aggregation AMG-PCG 7 iterations / 211 s (rel. resid 2.7e-5,
  plus a 451 s setup shared by both channels) — re-fill ≈ 15 min instead of 40–63 (pyamg installed
  into the env 2026-09-23; a new dependency if this is promoted). v4 combines parallel pairs + AMG
  re-fill + a single final fold census (four intermediate 77M-voxel censuses had cost ~22 min in
  v2 — census only the final field): same certificate (0/0/+0.0101, residual 1.02 px / 98.7%,
  `full_v4/report.json`), preprocessing 20 min (pins 3 s, pairs 5 min, matrix build 23 s, AMG setup
  9 min, CG 7+6 iterations in 2.3+2.8 min), 2D 7 min — but the run was **contended** by an
  unrelated job on the box, so 2.5D took 115 min (`wall_s` 6,921 s) against v2's 13, on an
  identical residual (1,959 vs 1,947 cells < 0). Uncontended estimate for the v4 chain: ≈ 20 + 5 +
  13 ≈ 40 min.
- **v5** (`cohort/`, `full_chain_v5.py <brain> <variant>`): `violating_pairs_pruned` — exact, finds
  a violating pair from its larger-`|d|` end with radius `min(radius, 2|d|/c)` — 70 s on the full
  volume vs 320 s (4 workers) / 12 min for the plain test, identical pairs. **Certificate conflict
  found and left diagnostic-only**: a final per-slice bilinear 2D pass after the 2.5D stage (298
  slices on B0039 carried sub-pixel bilinear folds) pushed 1,674 tets back under the 0.01 margin
  (all still ≥ 0, floor 706 instead of 0) — the bilinear (per-slice) and simplex-3D (tet)
  certificates are certificates of *different* interpolants, and each pass can break the other's
  margin. Simplex-3D is the certificate of record for a 3D field, so v5's stage F only counts the
  bilinear-fold slices; it does not run a corrective pass.

### 14.4 Full-volume result and cohort

Full B0039 (Laplacian-exterior, `(3, 528, 320, 456)`): raw 967,234 folds (floor 830,174, min
−312.38) → **0 folds, 0 floor, min +0.0101**, feasible. Landmark residual ≈ 1.0–1.03 px median,
≈ 98.7–98.9% within 10 px, L2 move from raw ≈ 17,850–17,900 across v1–v4 (identical certificate,
...
`chain2d.py` (`slices2d/results.json`) runs the per-slice chain (pin read → drop → 2D re-fill →
2D engine, no 2.5D) on the canonical plateau slices, comparing a 2D-stencil pin read against the
3D-stencil read used for volumes, and reads correspondences for the residual only:

| case | read | pins | true corr | dropped | folds raw→refilled→final | total s | L2 move | resid px | ≤10px |
|---|---|---|---|---|---|---|---|---|---|
| B0039_z1 | 2D | 1,096 | 20 | 1,015 | 3,975→347→0 | 16.1 | 22,647 | 105.5 | 0% |
| B0039_z1 | 3D | 100 | 20 | 83 | 3,975→310→0 | 18.8 | 42,835 | 52.4 | 0% |
| B0039_z11 | 2D | 1,579 | 131 | 1,344 | 3,135→252→0 | 23.7 | 4,916 | 10.6 | 48.1% |
| B0039_z11 | 3D | 155 | 131 | 124 | 3,135→613→0 | 21.6 | 8,904 | 6.7 | 77.9% |
| B0032_z1 | 2D | 2,986 | 132 | 2,766 | 4,641→242→0 | 21.8 | 7,010 | 57.6 | 0% |
| B0032_z1 | 3D | 382 | 132 | 335 | 4,641→469→0 | 27.1 | 13,279 | 26.4 | 23.5% |
| B0039_z240 | 2D | 1,807 | 975 | 690 | 1,087→68→0 | 8.4 | 262 | 0.76 | 99.8% |
| B0039_z240 | 3D | 941 | 975 | 496 | 1,087→55→0 | 9.3 | 294 | 0.51 | 99.6% |

All eight arms certify (0 folds) in 8–27 s — against 877–1,155 s for the canonical `isqp_l2`
engine on the same slices, whose landmark residuals were 132.7 px (B0039_z1), 8.0 px (z11) and
77.5 px (B0032_z1) (`docs/paper/results/2d_canonical/cohort/results.csv`): the 3D-read chain is
lower-residual on all three plateau slices, but its L2 move from the raw slice is 4–20x the engine's. The 3D-stencil read is smaller and lower-residual on every sparse-pin
slice (fewer, cleaner pins: B0039_z1 100 pins for 20 landmarks vs the 2D read's 1,096); on the
ordinary, pin-dense slice z240 the two reads agree closely. **Ruling**: the 2D chain is the right
tool only for 2D-native fields. A slice of a 3D-solved volume is shaped by its neighbour slices,
which a 2D re-fill cannot see, so a 2D read on it is smeared and a pin-poor slice gets rebuilt from
almost nothing — cohort slices instead get the 3D preprocessing once per volume (14.3–14.4), then
the ordinary per-slice 2D engine.

### 14.6 B0304: the three-layer diagnosis

B0304 is the one cohort brain that does not certify (raw 3,738,194 folds, min −399 — four times
any other brain). Three layers, peeled in order:

1. **False pins from a `|Δu|`-scaled residual.** Fixed `tau=0.7` detects 309,833 "pins" for
   210,625 correspondences (every other brain: pins ≈ correspondences), because the CG residual
   scales with the prescribed displacement magnitude (median `|d|` 34 px on B0304 vs 1.4 px on
   B0039) and sits right at the 0.7 threshold — roughly 100k false pins, driving 224M violating
   pairs. Measured precision/recall against the true correspondences: `tau=0.7` → 0.675/0.993;
   `tau=2.0` → 0.977/0.953 (205,424 pins ≈ 210,625 correspondences); `tau=3.0` → 1.000/0.908.
   B0039 at `tau=0.7` is unaffected (0.998/0.967). Non-pin `|Δu|` p99.9: B0304 0.78 px, B0039 0.12,
   B0213 0.08 — it scales with `|d|`. Fix: a self-calibrating **`tau = max(0.7, 10 × p98(|Δu|)
   over all voxels)`** — resolves to 0.7 on B0039/B0213 (p98 sits below any plausible pin fraction
   — pins are ~0.5% of voxels) and to 2.35 (2.0 used in practice) on B0304. `PIN_TAU=auto`
   implements it in v5.
2. **Even with clean pins** (`tau=2.0`, 205k), 82M violating pairs remain; the cover keeps only
   30k (15%); the re-fill still leaves 298,432 folds (233,000 < 0, min −10.3), concentrated at
   z 135–147.
3. **Root cause: per-slice landmark offsets.** The raw field's own per-slice *mean* displacement is
   smooth there (slice-to-slice jumps ≤ 0.4 px), but the prescribed correspondences' per-slice
   *medians* jump ±10–30 px between adjacent slices — the sequence runs (13,0) → (2,−7.5) → (8,−4)
   → (−10,3) → (−9,21) → (15,7) → … → (27,−7), integer-valued — each section's landmarks carry a
   different rigid offset, so every inter-layer pair contradicts over a 1-px z gap. No in-plane
   edit reconciles slices pinned 30 px apart except abandoning one side, which is exactly what the
   cover does. None of the other six brains shows anything like it.

Best arm found: `tau=2.0` / `c=0.5` — 8 folds < 0.01, 0 below zero (min +0.0028), floor 3;
landmark residual 9.65 px / 51.0% within 10 px; L2 move 125,656; 2.5D wall 2.5 h. The nearest arm
to a certificate, still **not certified**. (`tau=0.7`/`c=0.5`: 25 folds < 0.01, 1 < 0 at min
−0.0004, floor 15, residual 9.43 px / 51.6%, L2 move 110,842. `tau≥0.7`/`c=1` did not finish inside
the 6 h cap.) One `tau=2`/`c=0.5` run crashed in `correct_dvf_25d`'s all-diagonals finalize census
(MemoryError-class, materialising 77M-voxel arrays with two arms and their pools live) after the
2.5D sweep had already finished — that result was lost and the run had to be relaunched alone on
the box.

**Cohort verdict**: 6/7 brains certify identically (0 folds / 0 floor / +0.0101, ~1 px residual);
B0304 does not — a per-slice landmark-offset defect in its correspondence data, not a solver
limit.

### 14.7 Refuted alternatives

| lever | measured | verdict |
|---|---|---|
| Lipschitz projection LP (`lipschitz_lp.py`): keep every pin, L1-minimal move to `|Δd| ≤ c|Δx|` | HiGHS solves it fast (5.4M-row LP 20 s at `r=15`; 36M rows 97 s at `r=60` — OSQP/ADMM could NOT: 20,000 ADMM iterations left 622,496–826,922 violations unconverged, `lipschitz.log`). Result is bad regardless: `r=15` → 43,763 folds / min −40.73 (`lipschitz_lp_c1.0_r15.json`); `r=60` → 50,664 folds / min −34.61 (`lipschitz_lp_c1.0.json`), vs the drop's 1,263 / −1.22 | REFUTED — wrong pins moved to the EDGE of consistency leave whole contours at Lipschitz-1 gradients, and the interpolant folds between them; dropping the wrong information outright works, projecting it to the boundary of plausibility does not (`c<1` untested, not worth pursuing) |
| Crossing / order-reversal pair test `(Δx+Δd)·Δx < c|Δx|²` (`pins.crossing_pairs`, kept in the module as the rejected alternative) | On the B0039 slab: 8,082 folds at `c=0`, 5,593 at `c=0.5` — WORSE than the norm test's 1,272 | REFUTED — the norm test's extra dropped pairs are shear differences, and shear folds the interpolant too; keep `|Δd| > c|Δx|` |
| Affine detrend before the pair test (B0304) | Re-fill left 620k folds vs 570k without detrending | REFUTED — worse; B0304's pin set is rough at the 5-px scale (noise-dominated: 210–224M of ~230M near pairs still violate the norm test after removing a global affine trend, residual 22 px median), not a smooth large warp an affine model captures |
| 3D windowed engine on the re-filled slab residual (`run_engine.py`, `giant_workers=4`, L2 + rows, checkpointed) | Killed at the 5 h hard cap (`time_budget_s` is not checked inside a RAS sweep — same fact as 13.4/13.7). Whole slab 3,685 folds (1,263 at z<24, 623 at z=38 a re-fill seam artefact) → 3,457 after 3 Schwarz sweeps in ~1 h (one giant region, 32 tiles/sweep, ~20 min each), 0 new folds, L2 17.3 — z 9–20 cleared well, the dense cluster at z 21–23 (160–265/slice) untouched | REFUTED as the tool for this residual — one giant region, still SQP-iteration-count bound; the 2D + 2.5D chain closes the same slab in ≈5 min |
| Local harmonic re-fill (radius 2/4/8) | 64,316 / 58,352 / 46,757 folds left, vs 1,263 global (14.2) | REFUTED — the removed pins' influence falls off ~1/r; the edit is global or nothing |
| Parallel re-fill CG channels (two processes) | 5.4 s/iteration vs 1.7 s serial | REFUTED — the 77M-DOF sparse matvec is memory-bandwidth-bound; never parallelise channels, parallelise across pair-test chunks instead |

### 14.8 The fidelity trade

Every certified run buys the zero-fold certificate with a real, quantified departure from the
prescribed correspondences: the global re-fill moves the large majority of voxels (93% of the
slab, mean 2.9 px), and the cover drops a substantial share of the detected pins outright — ≈35%
kept on the six clean brains, as low as 6–18% kept on B0304's harder arms. The result is that
1–2% of pins on the clean brains land beyond 10 px of their prescribed position (98.2–98.9% land
within 10 px), and the landmark residual is 0.8–1.2 px median on those brains vs single-digit
nanometre-scale agreement on the raw, uncorrected field. This is not a defect to fix — it is the
cost of resolving contradictions that cannot otherwise be resolved — but it means the certificate
is never reported alone. Every table in this section, and `dvfopt/dvf/pins.py`'s own docstring,
carries the landmark residual (median px, % within 10 px) beside the fold count for exactly this
reason.
