...
- `dvf_origins/` — standalone harness (NOT part of the package; imports `dvfopt` only as a library) that generates sample DVFs per fold-origin mechanism — (1) Laplacian of corrupted correspondences, (2) dense weakly-regularized optimization (skimage flow, SimpleITK demons/FFD on the `data/mouse_brain` pair), (3) learned-field proxy + saved VoxelMorph/TransMorph loaders, (4) discretized SVF + real ANTs slice — and the fold-morphology table over them (`python -m dvf_origins {list, generate, sweep}`, self-check `pytest dvf_origins`). Case registry = `CASES` in its `__init__.py`; see its README
