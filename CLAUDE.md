...
- **phi flattening — TWO conventions exist, do not cross-mix them:**
  ...
  - So the split is **not** "simplex (2D)/simplex (3D) vs Jdet" — it is `Constraint.pack`, and that attribute is the only thing to trust. `dvfopt/core/schwarz/_common.py` is pack-agnostic (slices `(C, *shape)` arrays, never flattens).
