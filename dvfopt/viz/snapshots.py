...
def plot_step_snapshot(
    jacobian_matrix, iteration, neg_count, min_val, windows=None, label=None, jacobian_before=None
):
    ...
    if jacobian_before is not None:
        ...
        fig, (ax_b, ax_a) = plt.subplots(1, 2, figsize=(12, 4), constrained_layout=True)

        # BEFORE panel
        ax_b.imshow(jacobian_before[0], cmap=CMAP, norm=norm, interpolation=INTERP)
        ...
        ax_b.set_title(
            f"BEFORE \u2014 Iter {iteration}  |  neg={neg_b}  min={min_b:+.4f}", fontsize=9
        )
        ...
        ax_a.set_title(
            f"AFTER \u2014 Iter {iteration}  |  neg={neg_count}  min={min_val:+.4f}", fontsize=9
        )
    ...
    # constrained_layout=True so the figure is compatible with the
    # package theme (and we don't need a manual tight_layout call,
    # which conflicts with constrained_layout).
