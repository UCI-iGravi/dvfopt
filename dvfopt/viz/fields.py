...
# ---------------------------------------------------------------------------
def plot_initial_deformation(
    deformation_i, msample=None, fsample=None, figsize=(14, 6), quiver_scale=1
):
...
def plot_deformations(
    ...
    * Bottom-right: Corrected displacement quiver
    ...
    rows = [
        (
            "initial",
            np.min(deformation_i[2, 0]),
            np.max(deformation_i[2, 0]),
            np.min(deformation_i[1, 0]),
            np.max(deformation_i[1, 0]),
            np.min(jacobian_initial),
            np.max(jacobian_initial),
            init_neg,
        ),
        (
            "corrected",
            np.min(phi_corrected[1]),
            np.max(phi_corrected[1]),
            np.min(phi_corrected[0]),
            np.max(phi_corrected[0]),
            np.min(jacobian_final),
            np.max(jacobian_final),
            final_neg,
        ),
    ]
    ...
    axs[0, 0].imshow(jacobian_initial[0], cmap=CMAP, norm=norm, interpolation=INTERP)
    ...
    if msample is not None and fsample is not None:
        ...
        for i in range(len(msample)):
            my, mx = round(msample[i][1]), round(msample[i][2])
            my = np.clip(my, 0, H - 1)
            mx = np.clip(mx, 0, W - 1)
...
def plot_jacobians_iteratively(jacobians, msample=None, fsample=None, method_name="SLSQP"):
            ...
            if msample is not None and fsample is not None:
                ...
                for k in range(len(msample)):
                    ax.annotate(
                        ...
                        arrowprops=dict(
                            facecolor="black", shrink=0.05, headwidth=3, headlength=4, width=0.8
                        ),
                    )
        else:
    ...
    fig.colorbar(
        im, ax=axs_flat[:num].tolist(), orientation="vertical", fraction=0.03, pad=0.04, shrink=0.8
    )
...
def plot_deformation_field(
    ...
    if quiver_scale is None:
        ...
        axs[1].quiver(
            x, y, deformation[2, 0], -deformation[1, 0], scale=quiver_scale, scale_units="xy"
        )
