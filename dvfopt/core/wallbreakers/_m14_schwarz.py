...
def iterative_2d_tri_refine_repair_schwarz(
    ...
    **m14_kwargs
    ...
    for outer in range(max_outer_iters):
        ...
        # bboxes are cell-index space (fold_mask is (H-1, W-1)); the
        # inclusive span in cells is (cy1 - cy0 + 1), and the total cell
        # count is (H-1, W-1). The previous version compared
        # exclusive-cell-span against corner-count and could under-trigger
        # the global-m14 fallback for near-full clusters.
        if len(bboxes) == 1:
            b = bboxes[0]
            ...
            if (
                ...
                if verbose:
                    print(
                        ...
                        f' (HxW={H}x{W}): falling back to global m14',
                        flush=True,
                    )
        ...
        for b in bboxes:
            ...
            try:
                ...
                if verbose:
                    print(
                        f'  cluster {b["comp_id"]} FAILED: {type(exc).__name__}: {exc}', flush=True
                    )
                continue
        ...
        if post_n_neg >= prev_n_neg:
            ...
            if no_progress_rounds >= 2:
                if verbose:
                    print('  no progress for 2 rounds -> falling back to global m14', flush=True)
                history['fallback_to_global'] = True
                remaining = max(60.0, time_budget_s - (time.time() - t0))
                phi_out = iterative_2d_tri_refine_repair(
                    phi_out,
    ...
    if (
        ...
        if verbose:
            print(
                f'[final polish] min_T={post_min:+.4f} < threshold; running global barrier polish',
                flush=True,
            )
        t_polish = time.time()
        phi_out = iterative_2d_tri_barrier(
