...
def jdet_constraint_jacobian_2d(phi_flat, submatrix_size, exclude_boundaries=True):
    ...
    """
    sy, sx = (
        submatrix_size if isinstance(submatrix_size, tuple) else (submatrix_size, submatrix_size)
    )
    pixels = sy * sx
    shape = (sy, sx)
...
def shoelace_constraint_jacobian_2d(phi_flat, submatrix_size, exclude_boundaries=True):
    ...
    rows = np.concatenate([row_idx, row_idx, row_idx, row_idx, row_idx, row_idx, row_idx, row_idx])
    cols = np.concatenate(
        [
            dx_tl,
            dx_tr,
    ...
    return scipy.sparse.csr_matrix((vals, (rows, cols)), shape=(n_rows, 2 * pixels))
...
def triangle_constraint_jacobian_2d(phi_flat, submatrix_size, exclude_boundaries=True):
    ...
    return scipy.sparse.csr_matrix((vals, (rows, cols)), shape=(4 * n_cells, 2 * pixels))
